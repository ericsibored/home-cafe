...
-- eggs), so this supplies the missing image and the final description copy. BurntToast's $25 loaf is a
-- separate hard-coded listing in EventView.tsx with no menu_items row, so it is
-- unaffected.
-- ============================================================================

update public.menu_items mi
set description = 'Sticky toffee pudding meets banana bread, made with imported French butter and vanilla.',
    details     = details || '{"image":"/menu/banana-bread-slice.webp","imageFit":"cover"}'::jsonb
...
-- Verify
select mi.name, mi.description, mi.details->>'price' as price, mi.details->>'image' as image,
